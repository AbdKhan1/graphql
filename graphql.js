import { queryData, createLoader } from "./frontend.js"
const Url = "https://learn.01founders.co/api/graphql-engine/v1/graphql"
const ID = 559
const Username = "gymlad"
    ...
    fetch("https://learn.01founders.co/api/graphql-engine/v1/graphql", {
        ...
        .then(response => {
            if (response["data"]["transaction"].length > 0) {
                ...
                totalLevel["go-piscine"] = levelArr.filter(piscineGoLevel => piscineGoLevel["path"].startsWith('/london/piscine-go/'))
                ...
                totalLevel["js-piscine"]= levelArr.filter(julyJsLevel => julyJsLevel["path"].startsWith('/london/div-01/piscine-js') &&
                    new Date(julyJsLevel.createdAt).getTime() >= new Date("2022-05-01T00:00:00Z").getTime())
...
getTransactionData(Url)
    .then(response => {
        ...
        resultArr.push(levelArr[levelArr.length-1])
        for(let key in totalSkill){
            if (!skillz.includes(key)){
                totalSkill["total"] -= totalSkill[key]
                delete totalSkill[key]
            }
        }
        let skillSection = document.querySelector('.skills')
        let gradeSection = document.querySelector('.grades')
        let profileSection = document.querySelector('.profile')
        let xpSection = document.querySelector('.xp')
        let data = new queryData(resultArr)
        data.displaySkills(totalSkill,skillSection)
        data.displayProfile(profileSection)
        data.displayGrades(totalGrade,gradeSection)
        data.displayXp(xpSection)
    }
